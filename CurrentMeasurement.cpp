...
{
	//int motor_value;
	//*value = left ? last_motor_left : last_motor_right;
	//char plus = left ? (CURRENT_LEFT_ADC_CHANELL > CURRENT_RIGHT_ADC_CHANELL ? 1 : 0) : (CURRENT_RIGHT_ADC_CHANELL > CURRENT_LEFT_ADC_CHANELL ? 1 : 0);
	//uint16_t zero = left ? left_motor_null : right_motor_null;
	//measure_frequency((uint16_t*)adc_buff, MOTOR_DATA_LENGTH, value, plus, zero);
	//if (left)
		//last_motor_left = *value;
	//else
		//last_motor_right = *value;
	return 1;
}
